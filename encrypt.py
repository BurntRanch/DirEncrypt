...
with open('out.tmp', 'rb') as src:
    with open(path.join(files, 'out.ecd'), 'wb') as f:
        ...
        # ENCRYPTED ZIP FILE (dynamically sized) + SALT USED FOR ENCRYPTION KEY (16 bytes) + IV (16 bytes) + HASH (32 bytes) + SECOND HASH (32 bytes) + THIRD HASH (32 bytes) + IDENTIFIER (6 bytes)
        f.write(aes_obj.encrypt(content) + salt + aes_obj.iv + sha256(content).digest() + sha256(content[32:]).digest() + sha256(content[:32]).digest() + b'ECD1.2')
